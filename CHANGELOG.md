...
- A `fire_at` given as an ISO-8601 timestamp must now end in `Z` or an offset like `+02:00`; one without is refused with a 400 naming the field, where before it was read as UTC and could fire hours off the local time you meant. Epoch milliseconds are still accepted, and the editor is unaffected. Canceling a scheduled send now unlocks its post in the same write, so a failure can no longer leave the post locked with nothing scheduled.
