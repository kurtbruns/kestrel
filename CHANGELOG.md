...
- A Resend key that is invalid, revoked, or restricted, a sending domain that is not verified, or an SES account that is paused or suspended now pauses the send with nobody marked unsent, instead of recording the whole audience unsent (one SES request per recipient, each with the same error). The dashboard, the Sent page, and the send's watch show it as one alert carrying the provider's message, and the send resumes on its own once the account is fixed. This touches the database baseline, so rebuild the database as described in the first entry above.
