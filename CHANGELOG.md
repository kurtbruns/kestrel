...
- Stored settings that can no longer be read are no longer quietly replaced by the defaults, which the next save of any setting then wrote over your template and identity. Settings reads and saves now fail with a `settings_corrupt` error, the stored settings are left exactly as they were for you to repair, and the log says `SETTINGS_CORRUPT` with what is wrong. Scheduled sends still go out, since their emails were made when they were scheduled.
