...
- The public subscribe form now gives the same answer whatever state the address is in, so it no longer reveals who is on the list (SPEC §7). For API callers, `POST /subscribe` now returns `{"status":"check_inbox"}` in place of the subscriber's `status` and `action`.
...
- A confirmation email the provider refuses is no longer reported as sent. The subscribe form tells the reader it could not be sent and to try again in a few minutes, instead of to check their inbox; the dashboard's Add subscriber shows the provider's error; and the address is left as it was, instead of pending with a link nobody received (SPEC §7).
