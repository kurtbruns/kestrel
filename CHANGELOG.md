...
- The public subscribe form now gives the same answer whatever state the address is in, so it no longer reveals who is on the list: `POST /subscribe` returns `{"status":"check_inbox"}` instead of the subscriber's status and action. An address is sent at most one confirmation every 15 minutes, and a suppressed address is never sent one (SPEC §7). Add a Cloudflare rate-limiting rule on `/subscribe` to limit each client (see the setup guide's Access step). The dashboard's Add subscriber says when it sent nothing because the address is suppressed or was sent a confirmation moments ago, and `POST /subscribers` returns a null `subscriber` for a suppressed address that was never on the list. This touches the database baseline, so rebuild the database as described in the Fixed entry about sends through Resend to 100 or more subscribers.
