...
/** One route in the generated API reference (a view of a `RouteDef`, metadata only). */
...
/** The reference routes of one access tier, with a short heading for the tier. */
...
// The in-app API reference. Builds on the docs prose chrome, adding a compact row
// per route: a colored method chip, the path in mono, the access tier, a summary,
// and any hand-authored example bodies.
const REFERENCE_STYLE = `${DOC_STYLE}
.ref { max-width:820px; margin:0 auto; padding:32px 24px 64px; }
.ref > header { margin-bottom:8px; }
.ref h2 { text-transform:none; }
.ref .blurb { margin:0 0 16px; font-size:14px; }
.route { border:1px solid var(--line); border-radius:8px; padding:14px 16px; margin:12px 0; background:var(--card); }
.route-head { display:flex; align-items:center; gap:10px; flex-wrap:wrap; }
.method { font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace; font-size:12px; font-weight:700;
          letter-spacing:0.03em; padding:2px 8px; border-radius:5px; color:#fff; }
.method-GET { background:#15803d; }
.method-POST { background:#1d4ed8; }
.method-PUT { background:#b45309; }
.method-DELETE { background:#b91c1c; }
.route-head .path { font-size:14px; background:var(--bg); border:1px solid var(--line); border-radius:5px;
                    padding:2px 8px; word-break:break-all; }
.tier-tag { margin-left:auto; font-size:12px; color:var(--muted); border:1px solid var(--line);
            border-radius:999px; padding:1px 9px; }
.route .summary { margin:10px 0 0; font-size:15px; }
.route .desc { margin:6px 0 0; font-size:14px; }
.example { margin-top:10px; }
.example .ex-label { display:block; font-size:12px; font-weight:600; color:var(--muted); margin:8px 0 4px; }
.example pre { margin:0; }
`;

/** The generated API reference: the route manifest rendered read-only, grouped by
 *  access tier. Served through the authed `/api/reference` route and shown in the
 *  editor's API view — a same-origin authed fetch, never a top-level navigation.
 *  Every route here IS a registered route (src/reference/), so it can't drift. */
export function referencePage(opts: { appOrigin: string; groups: ReferenceGroup[] }): Response {
  const example = (label: string, value: unknown): string =>
    value === undefined
      ? ""
      : `<div class="example"><span class="ex-label">${escapeHtml(label)}</span>` +
        `<pre><code>${escapeHtml(JSON.stringify(value, null, 2))}</code></pre></div>`;

  const routeHtml = (r: ReferenceEntry): string =>
    `<div class="route">` +
    `<div class="route-head">` +
    `<span class="method method-${escapeHtml(r.method)}">${escapeHtml(r.method)}</span>` +
    `<code class="path">${escapeHtml(r.path)}</code>` +
    `<span class="tier-tag">${escapeHtml(r.access)}</span>` +
    `</div>` +
    `<p class="summary">${escapeHtml(r.summary)}</p>` +
    (r.description ? `<p class="desc muted">${escapeHtml(r.description)}</p>` : "") +
    example("Request", r.example?.request) +
    example("Response", r.example?.response) +
    `</div>`;

  const sections = opts.groups
    .map(
      (g) =>
        `<section><h2>${escapeHtml(g.title)}</h2><p class="blurb muted">${escapeHtml(g.blurb)}</p>` +
        g.routes.map(routeHtml).join("") +
        `</section>`,
    )
    .join("");
...
<body><article class="ref doc">
<header>
<h1>API reference</h1>
<p class="muted">Generated from the route registration — every endpoint the app and Claude can call. Base URL <code>${escapeHtml(opts.appOrigin)}</code>.</p>
</header>
${sections}
</article></body>
</html>`;
