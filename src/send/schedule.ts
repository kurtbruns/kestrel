...
export async function freeze(
  ...
  if (post.status !== "draft") throw conflict("post is not a draft");
  ...
  if (!revision) throw badRequest("post has no content to send");
  ...
  await env.DB.batch([
    env.DB
      ...
      .bind(id, post.id, fireAt, rendered.html, rendered.text, rendered.subject, audience.length, now),
    env.DB
      .prepare("UPDATE posts SET status = 'scheduled', updated_at = ? WHERE id = ? AND status = 'draft'")
      .bind(now, post.id),
  ]);
...
export async function cancel(env: AppEnv, sendId: string): Promise<SendRow> {
  ...
  if (!send) throw notFound("send");
  const now = Date.now();
  const res = await env.DB
    .prepare("UPDATE sends SET status = 'canceled', completed_at = ? WHERE id = ? AND status = 'scheduled'")
    .bind(now, sendId)
    .run();
  ...
  await env.DB
    .prepare("UPDATE posts SET status = 'draft', updated_at = ? WHERE id = ? AND status = 'scheduled'")
