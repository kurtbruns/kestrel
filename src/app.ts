...
export function createRouter(archiveBasePath: string): Router {
  ...
  const manifest: RouteDef[] = [
    ...
    },
    {
      ...
      description:
        "Updates only `fire_at` on a still-`scheduled` send: the frozen render is untouched (the audience is resolved when the send fires) and the review window is preserved. Distinct from cancel → edit → schedule again, which is for content changes. Same minimum lead as scheduling.",
      example: {
