...
4. **One vocabulary, one example set.** The documents share a single glossary (publisher versus developer) and a single set of example names (hostnames and the like). A rename updates that source; a straggler is drift. The newsletter entity is a **post**, and "issue" is reserved for the GitHub tracker: it never names a post, nor a delivery failure, which is a **bounce** or a **complaint**.
