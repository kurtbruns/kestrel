...
4. **One vocabulary, one example set.** The documents share a single glossary (publisher versus developer, post versus issue) and a single set of example names (hostnames and the like). A rename updates that source; a straggler is drift.
