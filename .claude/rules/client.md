...
- Layout: a feature folder per part of the publication — `posts/` (the drafts list, the editor, and its autosave, dirty, and revision logic), `sends/` (the list, the record, the shared `progress` helpers, the resolve and reschedule `dialogs`), `subscribers/` (the list, the add and unsubscribe `dialogs`), `settings/` (settings, the template page, the re-make flow), `dashboard/`, and `room/` (docs, the API reference, the room's `shell`) — each holding its views, its dialogs, and its pure logic. Everything cross-cutting stays flat in `client/`: the app (`main`, `state`, `router`, `shell`, `auth`, `api`, `dev_reload`), the primitives (`html`, `dom`, `icons`), the reads of the deployment reflection (`deployment`) and the sidebar brand (`brand`), and the shared pieces (`notice`, `savebar`, `widgets`, `list_controls`, `highlight`, `helpers`). Names inside a folder drop the folder's word (`sends/list.ts`). Specs sit beside their module; the harness is `client/test/`.
