...
- The app and the list views are strict; `docs`, `reference`, `dashboard`, `editor`, `settings`, and `template` still carry `// @ts-nocheck` from the split. `main.ts` is the entry (imports every module, runs `boot()`). Lift a pragma per module, with the wire types from `shared/`: a route's response shape is defined there and the Worker route is typed against it in the same change, so the compiler holds both ends.
