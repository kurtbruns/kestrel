...
Quality gate before finishing: `npm test`, `npm run typecheck`, and `npm run check`. Biome (`biome.json`) is the formatter + linter, on the same config as the sibling Worker projects. There is no CI in the repo: the gate is run by hand, and `npm run deploy` deploys by hand, per environment (through npm, not a bare `wrangler deploy`, so the build stamp below is fresh). `wrangler types` regenerates `worker-configuration.d.ts` (gitignored), so run `typecheck` after touching `wrangler.jsonc`.

The `recommended` preset is enforced at `error` everywhere; the tree is lint-clean. Three deliberate carve-outs: an `overrides` block turns off `noNonNullAssertion` + `noExplicitAny` for the specs only (`test/**` and `client/**/*.spec.ts`; tests legitimately assert known fixture shapes and type parsed JSON as `any`, and both stay enforced in `src/` and the client modules), a second override turns off `noImplicitAnyLet` for `client/main.ts` alone while it sits under `@ts-nocheck` (it is deleted with the pragma, see the client bullet below), and four intentional exceptions in `public/dashboard/styles.css` carry inline `biome-ignore` notes (the two deliberate `!important` rules and two descending-specificity selectors, where the cascade is decided by specificity, not source order). Reach for `unwrap(value, what)` (`src/lib/unwrap.ts`) instead of `!` for a row read back right after writing it — it fails loud with a name.
...
- **`db/` holds all SQL, and nowhere else does.** Until the first deployment, `migrations/0001_init.sql` is the whole schema and is edited in place: no database has ever run anything else, so there is nothing to migrate. Because wrangler tracks applied migrations by filename, a change to the baseline means every existing local database is rebuilt, not migrated: with `wrangler dev` stopped (a running one keeps the old database file open), delete `.wrangler/state/v3/d1` and run `migrate:local` (`migrate:local` alone sees nothing to apply and the app 500s on the missing column). From the first deployment on, `migrations/` is append-only: never edit a shipped migration, add a new one.
...
- **The admin SPA is authored in `client/` and built; `public/dashboard/app.js` is its output, never its source.** `scripts/build-client.mjs` bundles `client/main.ts` with esbuild into that one framework-free, dependency-free, plain-JS file (an IIFE, so the browser receives exactly what it always did), gitignored and headed `// GENERATED`: a hand patch to it works until the next build, then vanishes. The build runs at every entry that needs the bundle: `npm run dev` (once, then an esbuild watch beside wrangler that rebuilds on `client/` edits in ~10ms), wrangler's `build.command` (so a bare `wrangler deploy` can never ship a stale bundle; its `watch_dir` is pinned to `src/` so `wrangler dev` does not double-build client edits), and `pretest`. The build must stay deterministic, with no build-time defines and no environment-dependent output, because the committed `?v=` stamp is a hash of its bytes. `client/` has its own `tsconfig.json` (DOM lib, no Worker types) that `npm run typecheck` runs as a second program, and its specs run in the `client` Vitest project under happy-dom (`vitest.config.ts` splits the suite into `worker` and `client` projects, the workerd pool untouched). `client/main.ts` is the former `app.js` moved verbatim under `// @ts-nocheck`; the pragma is lifted module by module as the file is split, and every new client module is strict from its first line.
- **The admin static assets are fingerprinted, not hand-versioned.** `scripts/stamp-admin-assets.mjs` writes a content hash onto the `?v=` of `styles.css`/`app.js` in `public/dashboard/index.html`, and `public/_headers` caches those hashed URLs immutably. It runs after every client build (startup, each watch rebuild, `build.command`) and after a `styles.css` save under the watch, and, as `assets:check`, in `pretest` (so the gate catches an unstamped commit). Never hand-edit the `?v=`; `npm run assets:build` rebuilds and re-stamps by hand. In local dev the SPA polls `index.html` for a changed stamp (`client/dev_reload.ts`, on only when the boot probe reports auth mode `dev`, which no deployed env has): a new `app.js` stamp reloads the page and a new `styles.css` stamp hot-swaps the stylesheet. The poll fetches with cache mode `reload` on purpose: `wrangler dev` keeps serving a rewritten asset under its original ETag, so a plain reload would revalidate to a 304 and keep the stale page.
...
- **Keep the module boundaries** (above): SQL in `db/`, the one render path in `render/`, provider-specific code behind the `providers/` seam, the send state machine in `send/`, the admin SPA's source in `client/` (never the generated `app.js`).
