...
- **`send/` owns the send state machine.** `schedule.ts` freezes the render onto a `sends` row and soft-locks the post (I3, I6), pinning the template revision it used and refusing a schedule-again that would need a template choice it wasn't given (SPEC §6); the template update re-freezes the same row in place; `loop.ts` delivers in batches and marks each recipient in `deliveries` as accepted, so a retry or restart never re-mails anyone (I4); `sweep.ts` fires due sends and raises missed ones loudly.
- **`services/template_history.ts` is the one writer of the current email template (SPEC §9).** The template's history lives in `template_revisions` (`db/template_revisions.ts`); the settings blob keeps the current revision's id *and mirrors its html* so the render path keeps reading `settings.emailTemplate`. Those two are written in one batch by this module only (save, restore, and the first use that records the initial template as revision one), which is what keeps them from drifting. Never write `emailTemplate` any other way. A scheduled post's preview and test read the send's frozen copy, not a live render (SPEC §5): `routes/render_actions.ts` has the one loader.
