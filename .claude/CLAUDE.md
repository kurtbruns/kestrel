...
- **`services/template_history.ts` is the one writer of the current email template (SPEC §9).** The template's history lives in `template_revisions` (`db/template_revisions.ts`); the settings blob keeps the current revision's id *and mirrors its html* so the render path keeps reading `settings.emailTemplate`. Those two are written in one batch by this module only (save, restore, and the first-run backfill that records an unrecorded template as revision one), which is what keeps them from drifting. Never write `emailTemplate` any other way. A scheduled post's preview and test read the send's frozen copy, not a live render (SPEC §5): `routes/render_actions.ts` has the one loader.
...
- **`db/` holds all SQL, and nowhere else does.** `migrations/` is append-only from the `0001_init.sql` baseline (the pre-release history was squashed into it before any deployment) — never edit a shipped migration, add a new one. A local database created before the squash still works; to rebuild one from the baseline, delete `.wrangler/state/v3/d1` and run `migrate:local`.
