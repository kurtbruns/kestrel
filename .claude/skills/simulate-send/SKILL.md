...
2. **Pick the profile** the demo needs. The server's `SIMULATE_SENDS` decides it, fixed at start:
   ...
   - `ses`: one recipient a request, no idempotency key. Each send runs out of its daily quota once (the send halts, "provider refusing the account", and retries five minutes later) and loses one request in flight, so it ends **wedged** until someone uses **Resolve**. On the default `SUBREQUEST_BUDGET` (Workers Free) SES sends about 3 recipients a minute; start the server with `SIMULATE_SENDS=ses SUBREQUEST_BUDGET=1000 npm run dev` to model Workers Paid.
