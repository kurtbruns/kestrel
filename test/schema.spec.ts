...
describe("schema (0001_init)", () => {
  ...
  it("every send carries the template revision it was made with (NOT NULL)", async () => {
  ...
  it("enforces UNIQUE(send_id, email) on deliveries (idempotency backbone)", async () => {
    ...
    await env.DB.prepare(
      "INSERT INTO sends (id, post_id, status, fire_at, rendered_html, rendered_text, subject, template_revision, scheduled_at) VALUES ('sn1','p1','scheduled',?, '', '', '', ?, ?)",
    )
      .bind(now, tpl, now)
      .run();
    await env.DB.prepare(
  ...
  it("enforces one active send per post via the partial unique index", async () => {
    ...
    await env.DB.prepare(
      ...
      .bind(now, now)
      .run();
    const tpl = (await currentTemplateRevision(env.DB)).id;
    const insertSend = (id: string, status: string) =>
      env.DB.prepare(
        "INSERT INTO sends (id, post_id, status, fire_at, rendered_html, rendered_text, subject, template_revision, scheduled_at) VALUES (?, 'p2', ?, ?, '', '', '', ?, ?)",
      )
        .bind(id, status, now, tpl, now)
        .run();
