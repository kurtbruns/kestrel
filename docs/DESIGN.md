...
- **A scheduled post shows its lock where editing would happen.** The composer's formatting toolbar stays in view with its buttons disabled and one label beside them, "Cancel the schedule to edit", instead of disappearing; the fields are read-only. The Write and Preview tabs carry the same icon-and-word pairing as the settings toggles (§5), so a tab and a toggle read as one family.
