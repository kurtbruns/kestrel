...
- **A scheduled send carries its actions wherever it is listed** (the dashboard, Drafts, and the editor's `scheduled` banner, §1): Cancel, Reschedule, and, only when the template has changed since the send was made, Update, as equal-weight Ghost. A send made with an older template than the current one is marked as such in every place it appears (the `older template` flag, §3), so the mark and the action are never apart.
