...
**Brand accent — `--k-accent` is the one.** It is Kestrel's own action hue (a fixed slate-blue with `--k-accent-contrast` for text on it and `--k-accent-soft` for tinted fills), used by Primary and Secondary and nothing else. It is kept independent of the reader surface's theming (SPEC §8) — it is the app's own chrome identity, not the newsletter's. Because the accent is slate-blue, the lifecycle hues stay clear of blue so a status never reads as the action: **scheduled is a violet**, not a blue — the scheduled pill, the editor's scheduled banner, and the fire-time countdown on the scheduled cards all share it, and it is deliberately distinct from `--k-accent` so "this is queued" never looks like "click here."
