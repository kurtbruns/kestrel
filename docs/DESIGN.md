...
| **⑥ Post-save notice** | The consequence of a save that succeeded but left something for the reader to decide, with the actions that resolve it. Today: a template save's "N scheduled posts keep the template they were made with", each with **Update** and an **Update all** (SPEC §9). Persistent until acted on or the surface is left. | In flow, directly under the editor it concerns (`.tpl-kept`, the banner component's `info` variant stacked over a list). Never a toast (it carries an action), never the save bar (the save succeeded). |

The **editor-state banner** is one component with variants, merging what were two banners (`.sched-banner`, `.fresh-banner`): `info` is a neutral notice, `scheduled` marks a frozen/soft-locked post (SPEC §6), and `conflict` warns that the draft changed elsewhere (another tab, or Claude). The `scheduled` variant is about the send's time and its window, and carries only the two actions that act on those, **Reschedule** (move the fire time) and **Cancel**, as **equal-weight Ghost**: each acts on the pending send without editing its content, and none is the view's Primary (the editor's Primary is Schedule, and it is absent while locked). The send's *composition* (the template it was made with, the `older template` mark, and **Update**) lives one card down, in the editor's send card (§7), so the banner says one thing. The sent-status queue's scheduled card carries all three actions on one row; the dashboard's scheduled card stays link-only but still carries the mark (SPEC §8, DESIGN §7). A `conflict`'s two actions are **equal-weight Ghost** too — both choices lose something, so neither is dressed as Primary: Reload discards local edits, Keep editing overwrites the other writer's copy. Presenting one as the safe default would be a lie about the tradeoff.
...
**"No email provider configured" labeling.** When no real email provider is set up — internally the dev `fake` transport, which records a send but delivers nothing — a confirmation like "Test sent to 2 addresses" or "Queued — cancelable for 5 minutes" would otherwise read exactly like a real send. So the send-action confirmations (test, schedule, send now) carry a `(no email provider configured — nothing is delivered)` suffix on their Toast, and the Sent list carries a persistent `.muted` descriptor saying the same of the record itself. The user-facing copy deliberately avoids the internal "fake transport" term; **"No email provider configured" is the catch-all voice** for this state. Both read from the same read-only deployment reflection as the "Local dev" identity chip, and — like it and the dev-only "Open dashboard" shortcut noted in the intro — are structurally absent once deployed, where the provider is SES/Resend. This is chrome about *how* mail was sent, never part of the frozen post record (SPEC I3). A later pass will make this notice environment-aware (local / demo / production) and turn the new-user case — clicking "Send test email" before setup — into a dismissible notice that links to the setup docs; that is a different notification kind (an actionable, persistent notice, not a transient Toast) and gets its own pass.
...
| subscriber `pending` · send `older template` | `--warn` | amber |
| post `canceled` · subscriber `suppressed` | `--danger` | red |

The four lifecycle hues (draft gray, scheduled violet, sending blue, sent green) are *facts*, kept distinct from the `danger`/`warn`/`ok` feedback scales — a `scheduled` badge is neither good nor bad. Reusing a feedback scale is allowed only when the state genuinely carries that reading: `canceled`/`suppressed` → `danger`, and `pending` (awaiting double-opt-in) → `warn` amber — the same "one red, one amber" economy as above, not a conflation. The rule this encodes: `pending` takes `warn`, **never** `--status-sending` — a token doing double duty is exactly the drift this section exists to catch. The `older template` mark (a scheduled send made with a template revision older than the current one, SPEC §8) takes `warn` for the same reason `pending` does: it awaits the publisher's decision, keep or Update, and is neither an error nor a lifecycle state. It rides as a `.row-flag` beside the send wherever the send is listed (the Drafts status cell, the scheduled cards, the editor's send card) and is never the whole signal: the Update action sits beside it wherever the send carries its actions.
...
- **A scheduled send carries its actions where it is managed** (the editor and the Sent page's scheduled queue, §2): Cancel, Reschedule, and, only when the template has changed since the send was made, Update, as equal-weight Ghost. In the editor, Cancel and Reschedule ride the `scheduled` banner and Update rides the send card (below); on the queue card all three share one row. The dashboard's scheduled card and the Drafts row are link-only and lead to the editor. A send made with an older template than the current one is marked as such in every place it appears (the `older template` flag, §3), including those link-only places, so the mark is never missing where the action is.
- **The editor's send card is the model, drawn.** Scheduling makes the email from three inputs as they stand (SPEC §6), so the foot of the editor carries a card that says so in the spec's words. For a **scheduled** post it is titled "The copy that will send": one row for the template the send was made with (its save date, the `older template` mark and a sentence about Update when that template is older than the current one), **Send test email** (Secondary) and **Update** (Ghost) in its head, and a caption that says scheduling locked the post so it can't drift from what was reviewed, that a test sends this copy exactly as it will go out (its "view in browser" link works once the send fires), and that changing the post means cancel, edit, and schedule again. It replaces the actions bar, so a locked post has no orphaned Save or Schedule. For a **draft** it is titled "When you schedule": the current template (with its save date and a link to the template surface) and the identity that will be frozen, and a caption stating the rule. When the template has changed since the post was last made, the template row says so and that the publisher will choose which to use when they schedule, so the dialog's question is announced before it is asked. The identity row is shown only for a draft: identity is not versioned, so what a frozen copy carries cannot be read back and must not be guessed at.
- **The template choice is asked, never assumed.** When the template has changed since a post was last made (SPEC §6), the Schedule and Send now dialogs carry a two-option choice under their fields: **Keep the template it had** and **Use the current template**, each with its revision's save date, as a radio pair rendered as option cards with *neither* checked. A pre-selected default would be the silent choice by another name. The dialog's Primary stays live; pressing it without a choice shows a field validation (§2, home ④) under the options rather than disabling the button. The choice sends the revision id, so it stays exact if the template is saved in between; if the server refuses for want of the choice anyway, the dialog re-asks with the two revisions the server names. A first schedule, or one after which nothing changed, shows no choice at all.
- **A scheduled post's test says what it is.** The send card's caption says the test is the frozen copy that will fire and that its "view in browser" link works once the send fires (SPEC §5); the test dialog's lead sentence says the same. A draft's test dialog keeps its plain wording.
- **The template surface owns the consequence of a save.** A template save that leaves scheduled sends on the revision they were made with reports them in the post-save notice (§2, home ⑥) with Update per post and Update all; a save that changes nothing reports nothing. The template's history sits on the same surface: one row per revision, newest first, the current one labelled and every other with a Ghost **Restore**, which confirms first (it is a save, and it discards unsaved editor content) and then reports kept sends the same way. The note under the editor states the rule in the publisher's words: a change applies to posts scheduled from now on; a post already scheduled keeps the template it was made with (cancel and schedule again, or Update); a sent post never changes.
