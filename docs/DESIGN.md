...
The **editor-state banner** is one component with variants, merging what were two banners (`.sched-banner`, `.fresh-banner`): `info` is a neutral notice, `scheduled` marks a frozen/soft-locked post (SPEC §6), and `conflict` warns that the draft changed elsewhere (another tab, or Claude). The `scheduled` variant carries the schedule-management actions — **Update** (make the send again with the current template), **Reschedule** (move the fire time), and **Cancel** — as **equal-weight Ghost**: each acts on the pending send without editing its content, and none is the view's Primary (the editor's Primary is Schedule, and it is absent while locked). Update appears only when the send was made with an older template than the current one, and then always beside the `older template` mark (§3) and one sentence saying what Update does, so the mark, the reason, and the action are never apart. Those same actions ride the sent-status queue's scheduled card; the dashboard's scheduled card stays link-only but still carries the mark (SPEC §8, DESIGN §7). A `conflict`'s two actions are **equal-weight Ghost** too — both choices lose something, so neither is dressed as Primary: Reload discards local edits, Keep editing overwrites the other writer's copy. Presenting one as the safe default would be a lie about the tradeoff.
...
- **A scheduled send carries its actions where it is managed** (the editor's `scheduled` banner and the Sent page's scheduled queue, §2): Cancel, Reschedule, and, only when the template has changed since the send was made, Update, as equal-weight Ghost. The dashboard's scheduled card and the Drafts row are link-only and lead to the editor. A send made with an older template than the current one is marked as such in every place it appears (the `older template` flag, §3), including those link-only places, so the mark is never missing where the action is.
...
- **A scheduled post's test says what it is.** Beside the Send test email action of a locked post, one muted line says the test is the frozen copy that will fire and that its "view in browser" link works once the send fires (SPEC §5); the test dialog's lead sentence says the same. A draft's test dialog keeps its plain wording.
