...
- **A scheduled send carries its actions where it is managed** (the editor's `scheduled` banner and the Sent page's scheduled queue, §1): Cancel and Reschedule, as equal-weight Ghost. The dashboard's scheduled card stays link-only, so the actions live where the send is, not on every mention of it.
