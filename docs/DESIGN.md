...
- **The watch is the record page in its in-flight state**, not a second page (SPEC §8). It refreshes live while the send is `sending` and eases off once the send is only settling (SPEC §12), since nothing the publisher can act on changes during settling.
