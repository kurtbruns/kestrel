...
The record view's **in-flight watch** state (the same page while a send is still sending, SPEC §8) is read-only in the same sense — there is no draft and no save. It differs only in being **live**: it updates while the send is in flight and tears down those live updates on navigation, the same discipline as the editor's freshness check and the scheduled countdowns. It is observe-only but for one control — **Resolve**, the operator's adjudication of a wedged send (SPEC §11) — which is the view's Primary while it applies and absent otherwise; a live view is not a save model, so the watch still "picks neither."
