...
The **editor-state banner** is one component with variants, merging what were two banners (`.sched-banner`, `.fresh-banner`): `info` is a neutral notice, `scheduled` marks a frozen/soft-locked post (SPEC §6), and `conflict` warns that the draft changed elsewhere (another tab, or Claude). The `scheduled` variant carries the schedule-management actions — **Reschedule** (move the fire time), **Cancel**, and, only when the template has changed since the send was made, **Update** (§7) — as **equal-weight Ghost**: each acts on the pending send without editing its content, and none is the view's Primary (the editor's Primary is Schedule, and it is absent while locked). Those same actions ride the sent-status queue's scheduled card; the dashboard's scheduled card stays link-only (SPEC §8, DESIGN §7). A `conflict`'s two actions are **equal-weight Ghost** too — both choices lose something, so neither is dressed as Primary: Reload discards local edits, Keep editing overwrites the other writer's copy. Presenting one as the safe default would be a lie about the tradeoff.
