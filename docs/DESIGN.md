...
**Brand accent — `--k-accent` is the one.** It is Kestrel's own action hue (a fixed slate-blue with `--k-accent-contrast` for text on it and `--k-accent-soft` for tinted fills), used by Primary and Secondary and nothing else. It is kept independent of the reader surface's theming (SPEC §8) — it is the app's own chrome identity, not the newsletter's. Because the accent is slate-blue, the lifecycle hues stay clear of blue so a status never reads as the action: **scheduled is a violet**, not a blue — the scheduled pill, the editor's scheduled banner, and the scheduled cards (filled violet, with the fire-time countdown in the same hue) all share it, and it is deliberately distinct from `--k-accent` so "this is queued" never looks like "click here."
...
**The in-flight watch reuses these scales, it doesn't add a palette.** A send still sending opens the record view in its live state (§5), and its chrome is built entirely from tokens that already exist:

- The **phase pill** — the derived reporting phase (SPEC §11): progressing / retrying / backing-off / needs-attention / settling / complete — is a small pill whose tone maps onto an *existing* scale, never a new hue: the actively-sending phases wear the `--b-sending` amber, `settling` the same amber, `retrying` and `backing-off` the `warn` scale, `needs-attention` the `danger` scale, `complete` the `ok` scale, `scheduled` the `--b-scheduled` violet. It is a status reading, a sibling of the lifecycle badge, so it stays out of the semantic-feedback meaning even where a tone happens to coincide.
- The **two progress bars** carry the two phases of a send (SPEC §6), and — this is the load-bearing rule — **both are drawn on the same scale: the whole frozen audience is the denominator for each.** A **dispatch** bar (provider-accepted over the audience) filled with the `--b-sending` amber — the same "this is sending" cue as the pill and the active-send cards. Beneath it a **delivery** bar that pairs to dispatch's frontier rather than using its own denominator: a neutral **grey `accepted` segment** reaches exactly as far as the dispatch bar, and the `ok` **green `confirmed`** fill trails inside it as receipts arrive. Because green (confirmed) can never exceed grey (accepted), delivery always reads as *lagging* dispatch — the two frontiers advance together and the green chases the grey. This replaces an earlier version where the delivery bar was `confirmed / accepted` on its own denominator, which made it visually race *ahead* of dispatch even though a receipt can never precede acceptance. Green is the number that becomes the record's "Delivered" tile.
- The **active-send cards** — the dashboard's active-send widget and the Sent page's "In progress" row — take the `--b-sending` amber fill and border, exactly as the scheduled cards take the `--b-scheduled` violet (§2), so an in-flight send reads as "sending" at a glance and is visually distinct from both the queued (violet) and the finished (neutral) rows. Each carries a mini dispatch bar in the same amber.
