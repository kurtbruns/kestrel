...
- The **two progress bars** carry the two phases of a send (SPEC §6): a **dispatch** bar (provider-accepted over the audience) filled with the `--b-sending` amber — the same "this is sending" cue as the pill and the active-send cards — and a lighter **delivery** bar (webhook-confirmed over accepted) filled with the `ok` green, because a confirmed delivery is a good outcome and this is the number that becomes the record's green "Delivered" tile. The delivery bar is deliberately the quieter of the two: it lags, and it is secondary to knowing the hand-off is progressing.
