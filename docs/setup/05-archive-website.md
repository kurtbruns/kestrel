...
Out of the box, `ARCHIVE_ORIGIN` defaults to `APP_ORIGIN` and `MEDIA_PUBLIC_BASE` to `${APP_ORIGIN}/media` (resolved in `src/env.ts`). So archive URLs are `https://newsletter.example.com/newsletter/{slug}` and images serve from the Worker's own origin. A newsletter works end to end whether or not you have a separate website, and wherever that website is hosted. If that is fine, skip this section.

## Optional — surface the archive on your website's apex

If your website's apex is a **Cloudflare** zone, you can present the archive under your main domain — `example.com/newsletter/*` — so links carry your primary domain's trust and rank with the rest of your site.

This is a routing concern on the apex zone, not a second app:

1. On the **apex zone**, add a **Worker route** `example.com/newsletter/*` pointing at this same Worker. The most-specific route wins, so the static site keeps every other path.
...
- **Reverse proxy** (keeps the apex benefit): configure your host to forward `example.com/newsletter/*` to `https://newsletter.example.com/newsletter/*`, and set `ARCHIVE_ORIGIN` to `https://example.com`. The reader's address bar stays on the apex.
- **Redirect** (simplest, but forfeits the benefit): a `301` from `example.com/newsletter/*` to the app. Trivial to set up, but it sends the reader's address bar back to the app host, so links no longer read as your apex.
