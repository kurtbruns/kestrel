...
In the editor (`/admin/`), open a draft and use **Send test email** to deliver the rendered issue to an address you control. A test goes through the *same* render path as a real send (I5), so a clean test is a real guarantee, not a lookalike.
