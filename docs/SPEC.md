...
A second condition the app cannot absorb is **the provider refusing the account itself**: a revoked or rotated key, a sending domain that is not verified, an account the provider has paused or suspended. It is not transient and it is not about any recipient, so it is neither retried away nor recorded against the audience. The send keeps its place with nobody consumed, tries again each tick, and is raised on the status surface at once, as one condition carrying the provider's own words rather than a failure for every recipient. The app has not stopped the send, and nothing in the app is needed to restart it: the fix is in the provider's account or the deployment's secrets, and once it lands the send resumes on the next tick from where it stood, mailing no one twice (I4) and losing no one. The provider's words are shown as it gave them, and never carry a credential (§9).
