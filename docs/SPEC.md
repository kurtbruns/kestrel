...
The in-flight surface is **observe-only** but for one control: the watch reports, and the only action it offers is Resolve (below). It never pauses, throttles, or auto-halts a reviewed send — no automatic decision widens the audience, skips the window, or stops a send the operator approved. (Operator pause/resume and an automatic deliverability circuit-breaker on a bounce/complaint spike are conceivable, but they are new automation over a reviewed send and are deliberately left out until there is an explicit decision to add them.)
