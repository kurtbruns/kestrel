...
Scheduling **makes the email**. It does three things at once: it **freezes the render** into a new send (the one copy that is the review artifact, what will fire, and the archive, I3), it **soft-locks the post** so it can't drift from what was reviewed, and it **records the fire time**. The frozen copy captures three inputs as they stand at that moment: the post's content, the email template (§9), and the publication identity. The content reaches a scheduled send afterward only by cancel, edit, and schedule again. A change to the template or the identity **re-makes every scheduled send at once**, after the publisher confirms it, so no scheduled send is ever on an older template or identity than the one in use (§9). The fire time must be at least the **minimum lead** out; the freeze rejects anything closer, so every send, however requested, spends at least that long visible and cancelable (I6). The lead is one app-wide interval that the deployment sets: five minutes unless the developer names another, and never less than one minute in any environment, local development included, because the sweep that fires sends runs once a minute (below): a shorter lead would still be kept, but the tick, not the lead, would decide when the send fires, so the setting would claim a precision the system doesn't have. It is never more than a day either: the lead bounds only the least wait before a send, never how far out one may be scheduled, so a longer one would hold back every Send now and is almost certainly a value in the wrong unit. A setting outside those bounds stops the app rather than being clamped to them (§9). The lead is deploy-time configuration, never a preference, because the API is also Claude's door: a review window Claude could shorten through the API would be no guard against Claude. From then until it fires the send is **visible and cancelable** (I6): the publisher and Claude review it, test emails go to real inboxes, and if anything is wrong the publisher cancels it.
