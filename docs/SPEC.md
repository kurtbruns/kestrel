...
The form is public, so it must not become a way to learn who subscribes or to flood a stranger's inbox. **A subscribe request never reveals list membership:** it gets the same answer whether the address is new, pending, confirmed, unsubscribed, or suppressed. **Confirmations are rate-limited per address:** one goes out only when it is due, never to an address already confirmed, never to a suppressed one, and at most once per address in a fixed interval of minutes, however often the address is submitted; a repeat inside that interval sends nothing. How often any one client may submit the form is a limit set at the edge when the app is deployed, not in the app.
