...
There is one deliberate exception to "never a link toward admin," and it exists **only** on a local dev instance. Running the app locally, the front door a developer lands on is `/` — the same public landing page — and the editor sits at `/dashboard`, so a developer who wants the editor has to know the path. On a dev-shaped instance the reader surface therefore injects a small, clearly-marked "Open dashboard" shortcut into the landing page and archive index. This does not weaken §10: it is a *link, not a redirect* (the public page still serves everyone unchanged, nothing bounces); it is *presentation only* (the public/admin route gate is byte-identical in every environment — no boundary is forked); and it is gated by the exact predicate that enables the local dev credential (fake transport, no Access, dev secret present), which is the one case `/dashboard` is reachable without an Access wall because the editor auto-mints a dev token. In any deployed environment that predicate is false, the shortcut is structurally absent, and `/dashboard` is the Access login wall the reader surface must never point at.
