...
On a local dev instance only, every public page (the landing page, the archive index, each post page, and the subscribe, confirm, and unsubscribe pages) also carries a small, clearly marked shortcut into the editor, a developer convenience that is structurally absent once deployed. Whoever sees a local instance is its developer or publisher trying it out, never a reader, so the shortcut is on every page they reach, including the ones a link in a test email opens. On a post page it is browser-only chrome like the masthead, so it is never in the frozen copy or a sent email (I3). It is the one deliberate exception to never linking toward admin, allowed because a local instance has no access wall in front of the editor; §11 states the rule and the carve-out.
...
The email template is the single HTML layout that every post is sent inside, authored as a stylesheet plus a logic-less set of placeholders for the post's body and the publication identity. The placeholders are named in Hugo's style, `{{ .Post.Body }}`, `{{ .Publication.Name }}`, `{{ .Email.UnsubscribeURL }}`: data is dotted and capitalized, so it can't be mistaken for a word of the template language itself. The language has one construct beyond a placeholder: `{{ if .IsEmail }} … {{ end }}` marks a region as email-only, shown in every email and left out of the archived post (§5). It is a marked region, not a general conditional: `.IsEmail` is the one condition, there is no else, and regions don't nest. A template with an unclosed or stray region, a region inside another, another condition, an else, or the post body placeholder inside a region (the archived post would be empty) is rejected. A region must also wrap whole elements once the HTML is parsed: its two tags inside the same element, outside any attribute, and not loose in a table, where the parser moves content (possibly the post) into or out of it. A template that fails this only once parsed renders with the built-in default instead, as an invalid template does, and preview and test say so. An unsubscribe link only inside a region is fine, since every email carries it. The built-in and starting templates mark their footer's unsubscribe and view-in-browser links and the address as email-only, and keep "Powered by Kestrel" on the web. The render path fills it and inlines its CSS (mail clients strip stylesheets), so the presentation *inside* the email is the publication's. Because that render is frozen at schedule (I3) and is the one path preview, test, and send all share (I5), a clean test proves the send.
