...
Because the app is self-contained (§10), it serves its own reader-facing pages, not only per-issue archives. It carries the newsletter's identity — the publication name, tagline, and logo (§8) theme these pages, so the reader surface reads as *the publication*, not the tool. Three public pages, all indexable and none ever bouncing a visitor toward an admin path (§10):
...
The confirmation email's **wording** — its subject, the line above the button, the button's label, and an optional reassurance footer — is the operator's to edit, a runtime preference read and written through the same authenticated API as everything else (§8). Its structure is not: there is no layout choice, and the email always leads with the **publication identity** (logo, name, tagline) as a masthead, which degrades to nothing when no identity is set — a transactional first-touch opens with who it is before the ask, rather than carrying identity author-placed inside a body the way an issue does. The **confirm link** is inserted by the app and always present, a required field left blank falls back to a built-in default, and the HTML and plain-text bodies are generated together — so editing the copy can never produce a confirmation email that is wordless, misshapen, or missing the link that records consent (I1). It is transactional, not an issue: it does not use the issue email template (§8) and carries no unsubscribe link.
...
The pending Sends, each with its fire time and its frozen render, and — because acting on the review window is what makes it real (a window you can't see into or act on isn't one, I6) — the two actions that manage a pending send without editing its content: a **one-call cancel** (unschedule), and a **one-call reschedule** of the fire time (§6, which moves it without re-freezing). Both belong wherever the scheduled Send is offered as a status view — the sent-status queue and the editor's scheduled banner carry the pair; the dashboard's scheduled cards stay a glanceable, link-only summary that opens the editor to act.

### What have I sent, and how did it do?

Each sent issue is a read-only delivery record, not an editable draft — so it opens a **record view** rather than a locked editor. That view answers *how the send went*: the audience frozen at send time, and the delivery-outcome breakdown over it — delivered, bounced, complained, and failed — read from the per-recipient delivery rows, not just a summary count, and reconciling to the frozen audience. The record is the source of truth for "did it go," because the app is the only thing that knows what actually happened at delivery time.

The record keeps two layers on different clocks: the render and the audience are **frozen** at send time (I3), while the **delivery outcomes go on settling** as *this send's own* webhook events arrive — bounces and complaints land after completion (§11), so the breakdown is the current truth about this one send, never rewritten by another send or a later cleared suppression.

Beneath that breakdown it shows the **per-recipient rows themselves**, so "who bounced" is a look, not a download: it opens on the rows that went wrong (bounced, complained, failed), with a toggle to show the delivered rows or all of them, paginated and searchable by address, each row carrying the provider's error or detail and splitting a **hard** bounce (permanent; it suppressed the address) from a **soft** one (transient; counted, never suppressed) on the kind recorded for this send when the event landed — a frozen fact of the record, not a read of the current, clearable suppression list, so the split can't drift after the fact. Those rows read directly from the delivery record — the source of truth — on a separate read endpoint, deliberately *not* the cheap counter poll the live watch leans on (below), since an inspection surface isn't polled and can afford to be heavier. It links to the archived issue (the exact frozen copy readers received, I3) and still offers the whole per-recipient record as a **CSV export**, because a record is only inspectable if you can get at it (§11).

Wherever that record is compressed to a single glanceable number — the sent-issue list, the dashboard's recent sends — the number is **confirmed delivered** (with any complaints, bounces, or send-time failures called out with it, each by kind — never rolled into one sum, which would flatten a complaint into a retried failure and invite arithmetic that doesn't reconcile against the frozen audience), read from the same denormalized counters (below), so a send never reads as cleanly "delivered" in a list while its own record shows it bounced. The sent-issue list can be narrowed to just the sends with a delivery issue — a filter on those same counters, deliberately not a sortable column, so the newest-first order stays and no severity ranking is implied. "Delivered" everywhere means webhook-confirmed, never merely provider-accepted. This splits the authoring lifecycle in two for whoever is looking: the still-changeable **draft and scheduled** issues are the writing side, and a **sent** issue is a closed record on the dispatch side — a scheduled issue shows on both, as the cancelable draft it still is and as the pending send it has become. Once its send *fires*, though, an issue crosses fully to the dispatch side: while it is **sending** it is surfaced as an active send that opens the live watch (below), not as an editable, cancelable draft — the writing side never offers to edit or cancel an issue that is already going out, and opening one takes you to the watch rather than a locked editor. (This holds even though the post's own stored status is still "scheduled" until the send completes: the send's state, not the post's, decides how an in-flight issue is presented and routed.)

### Is a send happening right now?

A send in flight has a **live watch** — the same record page (§8 "what have I sent") in its in-flight state, so one page is the whole life of a send from first hand-off to settled archive. It shows the two phases as two bars — **dispatch** (provider-accepted over the audience) filling first, and a lighter **delivery** (webhook-confirmed over accepted) filling behind it as receipts arrive — plus a derived **phase** (below), a breakdown of the counts, and rough throughput and time-to-finish. It reads a cheap progress endpoint on a short poll while sending and a slower one while settling, and clears itself on navigation. The same live send also gets a compact home on the dashboard and an in-progress row above the sent records, so an active send is visible at a glance and one click deep. To keep that poll cheap at any list size, the per-recipient delivery rows (which stay the source of truth) are summarized into denormalized counters on the send, maintained in step with each recipient transition and each webhook and rebuildable from the rows at any time — so reading progress is a single-row read, not a scan of the whole audience.

### Is anything wrong right now?

A send still retrying, a scheduled send that missed its fire time, a bounce spike, a provider problem. This is the only thing that ever needs your attention, so it's the only thing that surfaces loudly. The **bounce spike** is a real signal, not an approximation: it reads a recent send's confirmed bounce count off the denormalized counters (above) over its frozen audience and fires when that rate reaches the provider's danger zone — around the 5% at which a sender is put under review — so it stays quiet through the ordinary trickle of bad addresses and speaks up only when deliverability is genuinely at risk. It is read-only reporting: it warns, it never throttles or halts a reviewed send (the automatic circuit-breaker that would is deliberately deferred, §11). One of these conditions carries an action rather than just an alarm: a send wedged on an ambiguous in-flight delivery (§11) shows its count *and* the control to resolve it, because a stuck send whose only remedy is raw SQL isn't really inspectable.
...
A settings surface holds the app's own runtime preferences — the ones with no home in a post or a subscriber: the default recipients the test-send flow pre-fills, the **publication identity** (name, tagline, logo) that themes the public reader surface and the author's dashboard, a **mailing address** for the email's footer, the **email template**, and the **wording of the double opt-in confirmation email** (§7). Two identities meet here, and they don't mix. The **sender** — the `From:` header and its sending domain — is deploy-time infrastructure (§10), the email's authenticated identity, and is never editable in-app. The **publication identity** is a preference: it themes the reader surface and dashboard, and — with the mailing address — is also available to the **email template**, the single HTML layout (authored with a `<style>` block and logic-less `{{ variables }}`) that every issue is sent inside. So the presentation *inside* the email is the publisher's: the render path fills the template with the post's body and the publication identity and inlines its CSS (mail clients strip `<style>`), and because that render is frozen at schedule (I3) and is the one path preview, test, and send all share (I5), a clean test proves the send and editing the template never changes an issue already scheduled. One rule the template cannot break: every email must carry an unsubscribe link, so setting a template that omits `{{ email.unsubscribeUrl }}` is rejected (as an empty subject is), and a template that somehow can't render falls back to the built-in default rather than shipping a broken or unsubscribe-less issue (I2). The logo is a single global asset stored under a reserved media key and served like any other image. All of it is read and written through the same authenticated API as everything else (one door, two clients), so Claude and the editor configure the app the same way. Configuration splits along one hard line: this surface holds **preferences and never secrets**. The provider choice, its credentials, the access configuration, and the origins are deploy-time infrastructure that lives in the environment and its secrets (documented in the operator setup guide), never in the database and never readable or writable through the admin API — so a compromised admin session can change a preference but can never reach a credential. For orientation the surface *shows* the deploy-time configuration read-only, next to a link to the guide that explains how to change it.
...
The in-flight surface is **observe-only** but for one control: the watch reports, and the only action it offers is Resolve (below). It never pauses, throttles, or auto-halts a reviewed send — no automatic decision widens the audience, skips the window, or stops a send the operator approved. A bounce spike is *reported* loudly (§8) but never *acted on*: the report is observation, not automation. (Operator pause/resume and an automatic deliverability circuit-breaker that would halt a send on a bounce/complaint spike are conceivable, but they are new automation over a reviewed send and are deliberately left out until there is an explicit decision to add them — the loud bounce-spike report is exactly the seam such a circuit-breaker would later hook into.)
...
- **SES is already the transport**, warmed and out of the sandbox behind the current Sendy install; this app **replaces Sendy**. Reuse or add a sending identity for the `send.` role (§10) on that SES account. Bounce and complaint events arrive via SNS.
