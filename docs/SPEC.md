...
Because the app is self-contained (§10), it serves its own reader-facing pages, not only per-post archives. It carries the newsletter's identity — the publication name, tagline, and logo (§8) theme these pages, so the reader surface reads as *the publication*, not the tool. Three public pages, all indexable and none ever bouncing a visitor toward an admin path (§10):
