...
- **Archive self-contained by default.** The archive origin defaults to the app's own origin, so `newsletter.example.com/newsletter/{slug}` is the archive URL out of the box, and the public archive index at `/` is served by the same Worker. The editor is static assets under `/dashboard/` (the legacy `/admin/` path 301-redirects there).
