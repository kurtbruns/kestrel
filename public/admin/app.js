...
function route() {
  ...
  const current =
    view === "sends" ? "#/sends" : view === "subscribers" ? "#/subscribers" : "#/posts";
...
async function renderEditor(id) {
  ...
  async function showPreview() {
    ...
    try {
      ...
      const res = await fetch(`/posts/${id}/preview`, {
    ...
    m.el.querySelector("#tGo").onclick = () =>
      busy(m.el.querySelector("#tGo"), "Sending…", async () => {
        ...
        try {
          ...
          m.close();
          toast(r.sent ? `Test sent to ${addr}` : "Send failed");
        } catch (e) {
          toast(e.message);
        }
      });
